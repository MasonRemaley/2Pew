...
/// for this. Good values tend to range between 2 and 3.
exp: f32 = 2.0,
...
current_intensity_linear: f32 = 0,

/// Adds an impulse, intensity ranges from 0 to 1. Larger impulses last longer, multiple impulses
/// accumulate.
pub fn add(self: *@This(), intensity_linear: f32) void {
    self.current_s = @min(
        self.current_s + std.math.clamp(intensity_linear, 0, 1) * self.max_s,
        self.max_s,
    );
}
...
pub fn set(self: *@This(), intensity_linear: f32) void {
    self.current_s = @max(self.current_s, @min(intensity_linear, self.max_s));
}

/// Updates internal state.
pub fn update(self: *@This(), delta_s: f32) void {
    const target_linear_intensity = self.current_s / self.max_s;
    self.current_intensity_linear = @min(
        self.current_intensity_linear + delta_s / self.attack_s,
...
pub fn intensity(self: *const @This()) f32 {
    // return std.math.pow(f32, self.current_intensity_linear, self.exp);
    return smootherstep(self.current_intensity_linear);
}
